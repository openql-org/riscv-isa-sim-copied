...
fprintf(stderr, "qtelep (rd)%ld, (rs1)%ld, (rs2)%ld, (qimm6)%ld\n", insn.rd(), insn.rs1(), insn.rs2(), insn.k_qimm6());
if (rs2_reg == 0) {
  initZeroStateOfSingleQubit(p->get_qubits(rs1_num), qbit_idx);
  STATE.qbit = qbit_idx;
} else {
  ...
  for (uint8_t i = 0; i < nqubits ; i++ ) {
    if ( (( RS2 >> i ) & 0x1) == 1 ) {
      ...
      initZeroStateOfSingleQubit(p->get_qubits(rs1_num), i);
    }
  }
}
serialize();
